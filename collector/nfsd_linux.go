...
var (
	...
	nfsdReplyCacheMissesDesc = prometheus.NewDesc(
	...
	nfsdReplyCacheNoCacheDesc = prometheus.NewDesc(
...
func (c *nfsdCollector) updateNFSdReplyCacheStats(ch chan<- prometheus.Metric, s *nfs.ReplyCache) {
	ch <- prometheus.MustNewConstMetric( nfsdReplyCacheHitsDesc, prometheus.CounterValue,
		float64(s.Hits))
	ch <- prometheus.MustNewConstMetric(
		nfsdReplyCacheMissesDesc, prometheus.CounterValue,
		float64(s.Misses))
	ch <- prometheus.MustNewConstMetric(
		nfsdReplyCacheNoCacheDesc, prometheus.CounterValue,
		float64(s.NoCache))
}
