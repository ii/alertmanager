...
func (p *Peer) reconnect() {
	...
	// try to reconnect all? or just one?
	// with users having probably <5 alertmanagers, doesn't seem necessary
	// to do this in parallel.
	for _, pr := range p.leftPeers {
		// No need to do book keeping on leftPeers here. If a
		// reconnect is successful, they will be announced in
		// peerJoin().
		if _, err := p.mlist.Join([]string{pr.Addr.String()}); err != nil {
			// successfulReconnectionsCounter
			// first return value is number of successful joins.
		} else {
			// failedReconnectionsCounter
		}
	}
}
...
func (p *Peer) peerJoin(n *memberlist.Node) {
	...
	if oldStatus == StatusFailed || oldStatus == StatusLeft {
		...
		p.leftPeers = removeOldPeer(p.leftPeers, pr.Name)
	}
}
...
func (p *Peer) peerLeave(n *memberlist.Node) {
	...
	// TODO: Do we want to differentiate between intentional and
	// unintentional leaving? Does it have consequences in this code?
	switch pr.status {
	case StatusLeaving:
		// TODO: Announce StatusLeaving.
		pr.status = StatusLeft
		pr.leaveTime = time.Now()
		p.leftPeers = append(p.leftPeers, pr)
	case StatusAlive:
