...
* The `procfs` and `sysfs` flags have been renamed to `--path.procfs` and `--path.sysfs` respectively.
...
* [IMPROVEMENT] Include overal health status in smartmon.sh example script #546
* [IMPROVEMENT] Include `guest_nice` in CPU collector #554
* [IMPROVEMENT] Add exec_boot_time for freebsd, dragonfly #550
* [IMPROVEMENT] Get full resolution for node_time #555
* [IMPROVEMENT] infiniband: Multiply port data XMIT/RCV metrics by 4 #579
* [IMPROVEMENT] cpu: Metric 'package_throttles_total' is per package. #657
